...
void ConnectButton::clicked() {
    ...
    connect_complete_connection =
        hwif->attach_connect_complete(sigc::mem_fun(*this,&ConnectButton::connect_complete));
...
void ConnectButton::user_prompt(Glib::ustring prompt,
    ...
    switch(responseType) {
      case neon::USER_YN:
        user_response = sigc::mem_fun(*this, &ConnectButton::user_yes_no_response);
        break;
      case neon::USER_STRING:
        user_response = sigc::mem_fun(*this, &ConnectButton::user_text_response);
        break;
      case neon::USER_INT:
        user_response = sigc::mem_fun(*this, &ConnectButton::user_number_response);
        break;
      case neon::USER_NONE:
        user_response = sigc::mem_fun(*this, &ConnectButton::user_none_response);
        break;
    }
...
void ConnectButton::user_text_response(int responseCode) {
    ...
    if(!text_input.empty()) { 
        ...
        window->hardwareInterface->respond_from_user(response, user_prompt_handle);
        user_prompt_handle.reset();
    }
}
