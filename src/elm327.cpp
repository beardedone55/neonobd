...
Elm327::init(std::shared_ptr<HardwareInterface> interface) {
    ...
    init_thread = std::make_unique<std::thread>([this]() {this->initThread();});  

    return init_signal;
}

Glib::ustring Elm327::getErrorString() const {
    return error_string;
}
...
*/
    ...
    command_thread = std::make_unique<std::thread>([this](){this->commandThread();});
...
void Elm327::init_done() {
    ...
    init_thread.reset();
    if(init_complete) {
        command_complete_connection = 
            command_complete_dispatcher.connect([this](){command_complete();});
        command_thread_exit_connection = 
            command_thread_exit_dispatcher.connect([this](){command_thread_exit();});
    }
    init_signal.emit(init_complete);
}


sigc::signal<void(const std::unordered_map<unsigned int, 
                  std::vector<unsigned char>>&)>
Elm327::signal_command_complete() {
    return command_complete_signal;
} 

void Elm327::sendCommand(unsigned char obd_address,
...
bool Elm327::isConnecting() const {
    return !!init_thread;
}
...
template<typename T>
static T getNext(std::queue<T>& q, std::mutex& m) {
...
void Elm327::commandThread() {
    while(!disconnect_in_progress) {
        ...
        while(!cmd_queue.empty() && !disconnect_in_progress) {
            ...
            command_complete_dispatcher.emit();  //Let main thread know there is a response on the queue.
        }
    }
...
template<typename T>
static void clear_queue(std::queue<T>& q) {
    while(!q.empty())
        q.pop();
}
...
bool Elm327::enable_echo() {
    return check_response(send_command("ATE1\r"));
}

bool Elm327::disable_echo() {
    return check_response(send_command("ATE0\r"));
}

bool Elm327::enable_headers() {
    return check_response(send_command("ATH1\r"));
}
...
bool Elm327::enable_spaces() {
    return check_response(send_command("ATS1\r"));
}

bool Elm327::disable_spaces() {
    return check_response(send_command("ATS0\r"));
}
