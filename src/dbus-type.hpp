...
using DBusBasicType = 
        std::variant<std::monostate, std::uint8_t, bool, std::int16_t, std::uint16_t, std::int32_t, 
                     std::uint32_t, std::int64_t, std::uint64_t, double, std::string>;

// DBusType represents all possible DBus types, including containers that may
// contain DBusType objects.  The DBusType is constructed from an sd_bus_message.
// (See systemd/sd-bus.h .)

class DBusType {
public:
    //construct DBusType from sd_bus_message.
    DBusType(sd_bus_message& msg) : 
        m_type_signature(sd_bus_message_get_signature(&msg, 0))
    {
        ...
        switch(type) {
            ...
            case 'r':
                ...
                break;
            default:
                throw std::invalid_argument("DBus message contains an invalid type.\n");
        }
    }
    ...
    DBusType operator[](const std::integral auto& i) const {
    ...
    DBusType operator[](const std::string& i) const {
        return getDict().at(i);
    }

private:
    using DBusVariant =
        std::variant<std::uint8_t, bool, std::int16_t, std::uint16_t, std::int32_t, 
                     std::uint32_t, std::int64_t, std::uint64_t, double, std::string,
                     std::vector<DBusType>, std::unordered_map<DBusBasicType, DBusType>>;
    ...
    static std::unordered_map<DBusBasicType, DBusType> read_dict(sd_bus_message& msg, const char type, std::string_view contents) {
        std::unordered_map<DBusBasicType, DBusType> result;
        while(sd_bus_message_enter_container(&msg, type, contents.data()) > 0) {
            DBusType key(msg);
            DBusType value(msg);
    ...
    void read_container(sd_bus_message& msg, std::string_view type, std::string_view contents) {
        sd_bus_message_enter_container(&msg, type[0], contents.data());
        char element_type;
        const char* element_contents;
        if(sd_bus_message_peek_type(&msg, &element_type, &element_contents) > 0) {
