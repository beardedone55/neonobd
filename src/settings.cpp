...

{
    ...
    btDeviceCombo = Gtk::Builder::get_widget_derived<
        ComboBox<Glib::ustring, Glib::ustring>>(ui, "bluetooth_device_combo");
    btDeviceCombo->signal_changed().connect(
        sigc::mem_fun(*this, &Settings::selectBluetoothDevice));

    btScanLabel = ui->get_widget<Gtk::Label>("bluetooth_scan_label");
    btScanProgress =
        ui->get_widget<Gtk::ProgressBar>("bluetooth_scan_progress");
